...
type
  TfrmSell = class(TForm)
    ...
    pnl_qty: TRzPanel;
    RzLabel16: TRzLabel;
    edtnum_qty: TRzNumericEdit;
    pnl_diskonrp: TRzPanel;
    RzLabel27: TRzLabel;
    edtnum_diskonrp: TRzNumericEdit;
    RzLabel28: TRzLabel;
    RzLabel29: TRzLabel;
    RzLabel30: TRzLabel;
    pnl_tour: TRzPanel;
    ...
    procedure edtnum_qtyKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure edtnum_diskonrpKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure cb_salesChange(Sender: TObject);
    procedure cb_salesKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure cb_nogroupKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure FormShow(Sender: TObject);
    procedure edt_passwdKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
    oldTotal: Double;
...
//   Writeln(F,' DISKON PEMBULATAN....Rp.'+FormatFloat('###,##0', edtnum_discbulat.Value));
   ...
   Writeln(F,' ----------------------------------- ');
   Writeln(F,' '+cb_cardbank.Text+' '+edt_cardno.text);
   Writeln(F,'');
...
{   Datamodule1.ZQryutil.Close;
   ...
   Datamodule1.ZQryutil.SQL.Text := 'select sum(round(feebiro*subtotal)),sum(round(feedrivertl*quantity)) from selldetail '+
...
{      if (vfeebiro<>'0') then Datamodule1.ZConnection1.ExecuteDirect('insert into operasional ' +
      ...
      if (vfeedriver<>'0') then Datamodule1.ZConnection1.ExecuteDirect('insert into operasional ' +
      '(idTrans,faktur,tanggal,waktu,kasir,kategori,keterangan,kredit) values ' +
      '(''' + idTrans + ''',' +
      '''' + FakturBaru + ''',' +
      '''' + FormatDateTime('yyyy-MM-dd',TglSkrg) + ''',' +
...
//      SubTotal := SubTotal - PenjualanDBGrid.Fields[9].AsFloat;
    ...
    Subtotal := getDatanum('sum(subtotal)','formsell'+standpos+' where ipv='+ Quotedstr(ipcomp) +' and nourut<>'+ PenjualanDBGrid.Fields[10].AsString );
    SubTotal := SubTotal + PenjualanDBGrid.Fields[9].AsFloat;
  end
  else SubTotal := 0;
  ...
  lbl_jumlbrg.Caption := 'Jumlah Barang : ' + Formatfloat(',0.###;(,0.###);0',getDatanum('sum(quantity)','formsell'+standpos+' where ipv='+ Quotedstr(ipcomp) ) ) + ' pcs';
end;
...
//  ClearTabel('formsell'+standpos+' where ipv='+Quotedstr(ipcomp));
  ...
  Fill_ComboBox_with_Data_n_ID(cb_nogroup,'select IDselltourgroup,concat(norental,if(norental="","","/"),nogroup) nogrp from selltourgroup where tgl=CURRENT_DATE order by nogrp','nogrp','IDselltourgroup');
  Fill_ComboBox_with_Data_n_ID(cb_sales,'select IDsales,nama from sales order by nama','nama','IDsales');
  ...
  pnl_passwd.Visible := false;
  pnl_payment.Visible := false;
  pnl_qty.Visible := false;
  pnl_diskonrp.Visible := false;
  pnl_tour.Visible := false;
...
begin
  Subtotal := getDatanum('sum(subtotal)','formsell'+standpos+' where ipv='+ Quotedstr(ipcomp));
  SellTxtSubTotal.Value := SubTotal;
  CalculateSell;
  ...
  begin
    ...
    DataModule1.ZConnection1.ExecuteDirect('update formsell'+standpos+' o, product p set o.faktur='+Quotedstr(SellTxtNo.Text)+', o.satuan=p.satuan where (o.kode=p.kode) and (o.ipv='+Quotedstr(ipcomp)+') ');
    ...
    SQL.Add('quantity,hargabeli,satuan,subtotal,'+ Quotedstr(idTrans) +',feebiro,feedrivertl,feerent from formsell'+standpos+' where ipv='+ Quotedstr(ipcomp) );
    ExecSQL;
     ...
     SQL.Add('select kode,(quantity * -1),satuan,hargabeli,harga,faktur,'+Quotedstr(memketerangan.Text)+',''penjualan'','''+idTrans+''',' + QuotedStr(UserName) + ',''' + FormatDateTime('yyyy-MM-dd',TglSkrg) + ''',''' + vwkt + ''',''' + KodeGudang + 'T'' from formsell'+standpos+' where ipv='+ Quotedstr(ipcomp) );
     ExecSQL;
  end;
...
begin
  ...
  begin
    ...
    SQL.Add('quantity,hargabeli,satuan,feebiro,feedrivertl,feerent from formsell'+standpos+' where ipv='+ Quotedstr(ipcomp));
    ExecSQL;
...
{        DataModule1.ZQryUtil.Close;
    ...
    SQL.Add('select kode,(quantity * -1),satuan,hargabeli,harga,faktur,''penjualan'','''+idTrans+''',''' + FormatDateTime('yyyy-MM-dd',TglSkrg) + ''',''' + KodeGudang + 'T'' from formsell'+standpos+' where ipv='+ Quotedstr(ipcomp));
    ExecSQL;
  end;
end;
...
//    ClearTabel('formsell'+standpos+' where ipv='+ Quotedstr(ipcomp));
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('insert into formsell'+standpos+' ');
      ...
      SQL.Add('faktur,kategori,merk,seri,feebiro,feedrivertl,feerent from selldetail');
...
begin
  ...
  DataModule1.ZConnection1.ExecuteDirect('update formsell'+standpos+' set totalharga=round(quantity*harga),diskonrp=round((round(quantity*harga)*diskon*0.01)+diskon_rp)'+',subtotal=round(round(quantity*harga)-round((round(quantity*harga)*diskon*0.01)+diskon_rp)) where ipv='+Quotedstr(ipcomp));
...
//   if CekMaxStock=false then exit;
  ...
  begin
    ...
    Close;
    SQL.Clear;
    SQL.Add('select faktur from formsell'+standpos+' ');
  ...
  try
    ...
    if edt_bayarcard.Value<>0 then PrintStruck(CetakFaktur);

    SellBtnAdd.Enabled:=false;

    InfoDialog('Transaksi Faktur ' + CetakFaktur + ' berhasil!'+#13+#13+'Tunai       Rp.'+edt_bayartunai.Text+#13+'Card         Rp.'+edt_bayarcard.Text+#13+#13+'Kembali  '+SellLblkembali.Caption);
...
{  if Key = VK_F1 then
  begin
    ...
    frmSrcProd.SearchTxtSearch.Text := trim(edt_kode.Text);
    frmSrcProd.ShowModal;

    edt_kode.Text := trim(frmSrcProd.SearchTxtSearch.Text);
...
begin
 ...
 VK_RETURN : begin
              ...
              if pnl_payment.Visible then
              begin
               pnl_payment.Visible := false;
               edt_kode.SetFocus;
               edt_kode.SelStart := length(edt_kode.Text);
               exit;
              end;
 ...
 VK_PRIOR  : begin  //ReprintNota;
              edt_passwd.Text := '';
              pnl_passwd.Visible := true;
              edt_passwd.SetFocus;
             end;
 VK_UP     : DataModule1.ZQryFormSell.Prior;
 VK_DOWN   : DataModule1.ZQryFormSell.Next;
 VK_DELETE : if Datamodule1.ZQryFormSell.IsEmpty=false then
             begin
              SubTotal := SubTotal - DataModule1.ZQryFormSellsubtotal.Value;
              DataModule1.ZQryFormSell.Delete;
              CalculateGridSell;
 ...
 VK_F8 :     if Datamodule1.ZQryFormSell.IsEmpty=false then
             begin
              pnl_qty.Visible := true;
              edtnum_qty.Value := Datamodule1.ZQryFormSellquantity.Value;
              edtnum_qty.SetFocus;
             end;

 VK_F10 : if Datamodule1.ZQryFormSell.IsEmpty=false then
             begin
              pnl_diskonrp.Visible := true;
              edtnum_diskonrp.Value := Datamodule1.ZQryFormSelldiskon_rp.Value;
              edtnum_diskonrp.SetFocus;
             end;

 VK_F11 :  begin
            Fill_ComboBox_with_Data_n_ID(cb_nogroup,'select IDselltourgroup,concat(norental,if(norental="","","/"),nogroup) nogrp from selltourgroup where tgl=CURRENT_DATE order by nogrp','nogrp','IDselltourgroup');
            pnl_tour.Visible := true;
            cb_nogroup.SetFocus;
           end;
...
begin
  ...
  begin
    ...
    SQL.Add('where s.idsell='+ getdata('max(idsell)','sellmaster where left(faktur,2)='+Quotedstr(standPos)+' and isposted<>-1') );
    Open;
  end;

//  ClearTabel('formsell2'+standpos+' where ipv='+ Quotedstr(ipcomp) );
  DataModule1.ZConnection1.ExecuteDirect('truncate formsell2'+standpos);

  DataModule1.ZConnection1.ExecuteDirect('insert into formsell2'+standpos+' '+
  ...
  DataModule1.ZQryformsell2.SQL.Text := 'select * from formsell2'+standpos+' where ipv='+Quotedstr(ipcomp)+' order by kode ';
...
begin
 ...
 edt_tour.Text := trim(cb_nogroup.Text) + '  ' + getData('namatour','selltourgroup where tgl=CURRENT_DATE ');
end;
...
procedure TfrmSell.edtnum_qtyKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if key=VK_RETURN then
 begin
  Datamodule1.ZQryFormSell.Edit;
  Datamodule1.ZQryFormSellquantity.Value := edtnum_qty.Value;
  Datamodule1.ZQryFormSell.CommitUpdates;
  CalculateGridSell;
  Datamodule1.ZQryFormSell.CommitUpdates;
...
procedure TfrmSell.edtnum_diskonrpKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if key=VK_RETURN then
 begin
  Datamodule1.ZQryFormSell.Edit;
  Datamodule1.ZQryFormSelldiskon_rp.Value := edtnum_diskonrp.Value;
  Datamodule1.ZQryFormSell.CommitUpdates;
  CalculateGridSell;
  Datamodule1.ZQryFormSell.CommitUpdates;
...
procedure TfrmSell.cb_salesChange(Sender: TObject);
begin
 edt_sales.Text := trim(cb_sales.Text);
end;

procedure TfrmSell.cb_salesKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmSell.cb_nogroupKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if (key=VK_RETURN)or(key=VK_ESCAPE) then
 begin
...
procedure TfrmSell.edt_passwdKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if (key=VK_RETURN)or(key=VK_ESCAPE) then
 begin
  ...
  if (key=VK_RETURN) then
  begin
   if (isdataexist('select IDuser from user where password='+Quotedstr(trim(edt_passwd.Text))+' and IDusergroup<4')) then ReprintNota
   else errordialog('Maaf, Password ini tidak punya hak akses untuk Reprint Nota!');
  end;
