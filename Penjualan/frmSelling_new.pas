...
type
  TfrmSell = class(TForm)
    ...
    RzPanel9: TRzPanel;
    RzLabel22: TRzLabel;
    btnpaymentclose: TAdvSmoothButton;
    ...
    procedure btnpaymentcloseClick(Sender: TObject);
...
//   Writeln(F,' DISKON PEMBULATAN....Rp.'+FormatFloat('###,##0', edtnum_discbulat.Value));
   Writeln(F,'        ' + Floattostr(totqty)+' Pcs Kembali : Rp.'+FormatFloat('###,##0', edt_kembali.Value));
   Writeln(F,' ----------------------------------- ');
...
var
  FakturBaru,idTrans,GrandTotal,totpayment,vwktt,vfeebiro,vfeedriver: string;
  islunas: integer;
  vtotpayment: double;
begin
...
//      SubTotal := SubTotal - PenjualanDBGrid.Fields[9].AsFloat;
  ...
  lbl_jumlbrg.Caption := 'Jumlah Barang : ' + Formatfloat(',0;(,0)',getDatanum('sum(quantity)','sparepart.formsell where ipv='+ Quotedstr(ipcomp) ) ) + ' pcs';
end;
...
begin
  ...
  begin
    ...
    SQL.Add('(faktur,tanggal,waktu,kasir,IDsales,IDselltourgroup,IDcustomer,subtotal,discbulat,bayartunai,bayarcard,cardbank,cardno,cardname,kembali,');
    ...
    SQL.Add('quantity,hargabeli,satuan,subtotal,idsell,feebiro,feedrivertl)');
    SQL.Add('select faktur,kode,nama,kategori,merk,seri,harga,diskon,diskonrp,diskon_rp,');
    SQL.Add('quantity,hargabeli,satuan,subtotal,'+ Quotedstr(idTrans) +',feebiro,feedrivertl from sparepart.formsell where ipv='+ Quotedstr(ipcomp) );
    ExecSQL;
...
begin
  ...
  begin
    ...
    SQL.Add('quantity,hargabeli,satuan,feebiro,feedrivertl)');
    SQL.Add('select faktur,kode,nama,kategori,merk,seri,harga,diskon,diskon_rp,diskonrp,');
    SQL.Add('quantity,hargabeli,satuan,feebiro,feedrivertl from sparepart.formsell where ipv='+ Quotedstr(ipcomp));
    ExecSQL;
...
begin
  ...
  begin
    ...
    begin
      ...
      SQL.Add('faktur,kategori,merk,seri,feebiro,feedrivertl)');
      ...
      SQL.Add('faktur,kategori,merk,seri,feebiro,feedrivertl from sparepart.selldetail');
...
begin
  ...
  if bufstr='' then exit;
   ...
   DataModule1.ZQrySearchProduct.SQL.Strings[2] := 'where (p.kode='+Quotedstr(bufstr)+')or(p.barcode='+Quotedstr(bufstr)+')or(p.nama='+Quotedstr(bufstr)+') ';
   ...
   begin
    ...
    begin
     ...
     DataModule1.ZQryFormSelldiskon_rp.Value := getDataNum('t.diskonrp','sparepart.diskondet t left join sparepart.diskon d on t.IDdiskon=d.IDdiskon where d.isactive=1 and t.minqty<='+floattostr(vqty)+' and t.maxqty>='+floattostr(vqty)+' and t.IDproduct='+DataModule1.ZQrySearchProductIDproduct.AsString+' and '+Quotedstr(getmysqldatestr(Tglskrg))+' between d.tglawal and d.tglakhir ');
...
begin
 ...
 VK_ESCAPE : btnpaymentcloseClick(Sender);

 VK_F1 :     begin
              frmSrcProd.formSender := frmSell;
              frmSrcProd.SearchTxtSearch.Text := edt_kode.Text;
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select s.*,l.nama nmsales,c.nama nmcust,g.nogroup from sparepart.sellmaster s left join sparepart.sales l on s.idsales=l.idsales ');
  ...
  nmfile := vpath + 'struk.txt';
   ...
   begin
    ...
    Writeln(F,'         '+FormatFloat('###,##0',DataModule1.ZQryformsell2quantity.Value)+' X @Rp.'+FormatFloat('###,##0',DataModule1.ZQryformsell2harga.Value)+' = Rp.'+FormatFloat('###,##0',DataModule1.ZQryformsell2quantity.Value*DataModule1.ZQryformsell2harga.Value));
...
//   Writeln(F,' DISKON PEMBULATAN....Rp.'+FormatFloat('###,##0', edtnum_discbulat.Value));
   Writeln(F,'        ' + Floattostr(totqty)+' Pcs Kembali : Rp.'+FormatFloat('###,##0', DataModule1.ZQrySellMasterkembali.Value));
   Writeln(F,' ----------------------------------- ');
...
procedure TfrmSell.btnpaymentcloseClick(Sender: TObject);
begin
              pnl_payment.Visible := false;
              edt_kode.SetFocus;
