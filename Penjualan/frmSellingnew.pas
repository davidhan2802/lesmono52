...
{      DataModule1.ZQryFormSell.CommitUpdates;
 ...
 if (Key=VK_PRIOR) then ReprintNota;

 if (Key=VK_F8) then
 begin
  if Datamodule1.ZQryFormSell.IsEmpty=false then
  begin
...
begin
  ...
  nmfile := vpath + 'struk.txt';
   ...
   begin
    ...
    Writeln(F,'         '+FormatFloat('###,##0',DataModule1.ZQryformsell2quantity.Value)+' X @Rp.'+FormatFloat('###,##0',DataModule1.ZQryformsell2harga.Value-DataModule1.ZQryformsell2diskon_rp.Value)+' = Rp.'+FormatFloat('###,##0',DataModule1.ZQryformsell2subtotal.Value));
    Writeln(F,'');
