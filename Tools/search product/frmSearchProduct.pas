...
//  if formSender = frmSell then SearchBtnSearchClick(self)
//  else
//  begin

   if Trim(SearchTxtSearch.Text) = '' then
      SearchBtnClearClick(Self)
   else SearchBtnSearchClick(self);
//  end;

   SearchTxtSearch.SetFocus;
   SearchTxtSearch.SelStart := length(SearchTxtSearch.Text);
end;
...
//      SQL.Strings[1] := 'where a.kodegudang = '+ QuotedStr(KodeGdg) +' and b.' + SearchCategories + ' like ' + QuotedStr(SearchTxtSearch.Text + '%') + ' '; //and a.qty <> 0';
       SQL.Strings[0] := 'select p.*,cast("" as char(20)) faktur,p.'+kodegudang+' qty,p.'+kodegudang+'T qtytoko,p.'+kodegudang+'R qtybad,g.nama kategori from product p left join golongan g on p.IDgolongan=g.IDgolongan ';
  ...
  begin
   (SearchDBGrid.columns[5] as TColumn).fieldname := 'hargabeli';
   (SearchDBGrid.columns[5] as TColumn).Title.Caption := 'Harga Beli';
  end
  else
  begin
   (SearchDBGrid.columns[5] as TColumn).fieldname := 'hargajual';
   (SearchDBGrid.columns[5] as TColumn).Title.Caption := 'Harga Jual';
  end;
  ...
  begin
   { with DataModule1.ZQrySearch do
    begin
      Close;
      SQL.Clear;
      SQL.Add('select kode from formsell where ipv='+Quotedstr(ipcomp));
    ...
    vqty := 1;
    DataModule1.ZQryFormSellquantity.Value := vqty;
...
{      qtytamp:=0;
  ...
  begin
    ...
    DataModule1.ZQryFormBuybarcode128.Value := DataModule1.ZQrySearchProductbarcode128.Value;
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('select * from purchaseorderdetform ');
      SQL.Add('where ipv='+Quotedstr(ipcomp)+' and kode = ' + QuotedStr(DataModule1.ZQrySearchProductkode.Value) + ' ');
      Open;
      if (not IsEmpty) then
      begin
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('select * from terimagudangdetform ');
      SQL.Add('where ipv='+Quotedstr(ipcomp)+' and kode = ' + QuotedStr(DataModule1.ZQrySearchProductkode.Value) + ' ');
      Open;
      if (not IsEmpty) then
      begin
