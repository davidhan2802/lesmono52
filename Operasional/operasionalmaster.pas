...
uses Data, SparePartFunction, frmOperasional, U_cetak;
...
begin
  ...
  begin
    ...
    lbl_title.Caption := 'Operasional Kasir';
    OprTxtFilter.ReadOnly := true;

    lbl_lr.Caption := 'Struk';
  end
  else
  begin
    ...
    lbl_title.Caption := 'Operasional';
    OprTxtFilter.ReadOnly := false;

    lbl_lr.Caption := 'Laba/Rugi';
  end;
...
begin
  ...
  strstandpos := '';
  if usergroup='KASIR' then strstandpos:='left(faktur,2)='+Quotedstr(standpos)+' and ';
  ...
  begin
    ...
    SQL.Add('where '+strstandpos+' tanggal < ''' + FormatDateTime('yyyy-MM-dd',OprTxtSearchFirst.Date) + ''' '); //and kategori<>''%TRANSFER%''');
    Open;
    ...
    SQL.Add('where ' + strstandpos + SearchCategories + ' like ''' + OprTxtSearch.Text + '%' + '''');
    if UserGroup = 'KASIR' then
    begin
...
//      SQL.Add('and kasir = ''' + UserName + '''');
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select sum(debet - kredit) from formkeuangan');
    SQL.Add('where ' + strstandpos + SearchCategories + ' like ''' + OprTxtSearch.Text + '%' + '''');
    if UserGroup = 'KASIR' then
    begin
      SQL.Add('and tanggal >= ''' + FormatDateTime('yyyy-MM-dd',OprTxtSearchFirst.Date) + '''');
...
var
  FilterView, SisaSaldo, strstandpos: string;
begin
  case OprTxtFilter.ItemIndex of
  0: FilterView := '';
  1: FilterView := 'and kategori = ''' + 'OPERASIONAL' + '''';
  ...
  strstandpos := '';
  if usergroup='KASIR' then strstandpos:=' and left(faktur,2)='+Quotedstr(standpos);
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add('select sum(debet-kredit) as saldo from operasional');
    SQL.Add('where tanggal < ''' + FormatDateTime('yyyy-MM-dd',OprTxtSearchFirst.Date) + ''' '+strstandpos); //and kategori<>''%TRANSFER%'' '+strstandpos);
    Open;
    ...
    begin
      ...
      SQL.Add('and tanggal <= ''' + FormatDateTime('yyyy-MM-dd',OprTxtSearchLast.Date) + ''' '+strstandpos);
//      SQL.Add('and kasir = ''' + UserName + '''');
      SQL.Add(FilterView)
    end
    else
...
procedure TFrmoperasionalmaster.PrintStruck;
var
  F : TextFile;
  i : integer;
  nmfile : string;
begin
  nmfile := vpath + 'opkasir.txt';
   ...
   Writeln(F,' =================================== ');
   Writeln(F,' No.'+ DataModule1.ZQryFormOperasionalfaktur.value);
   Writeln(F,' Tgl.'+FormatDatetime('dd/mm/yyyy',DataModule1.ZQryFormOperasionaltanggal.value));
   Writeln(F,' ----------------------------------- ');

   if DataModule1.ZQryFormOperasionaldebet.value<>0 then
   begin
      Writeln(F,' Telah Diterima Uang sebesar:');
      Writeln(F,'                Rp.'+FormatFloat('###,##0', DataModule1.ZQryFormOperasionaldebet.value));
   end
   else
   begin
      Writeln(F,' Telah Dikeluarkan Uang sebesar:');
      Writeln(F,'                Rp.'+FormatFloat('###,##0', DataModule1.ZQryFormOperasionalkredit.value));
   end;

   Writeln(F,' Sebagai :');
   Writeln(F,' '+DataModule1.ZQryFormOperasionalketerangan.value);
   Writeln(F,'');
   Writeln(F,' Diterima dengan baik dan benar, oleh :');
   Writeln(F,'');
   Writeln(F,' Yg Menerima,           Yg Menyerahkan,');
   Writeln(F,'');
   Writeln(F,'');
   Writeln(F,'');
   Writeln(F,' (-----------)          (------------)');
...
begin
  ...
  if usergroup='KASIR' then
  begin
   if DataModule1.ZQryFormOperasionalkategori.Value='OPERASIONAL' then PrintStruck;
   exit;
  end;
...
procedure TFrmoperasionalmaster.FormShow(Sender: TObject);
begin
  PanelOperasional.Color := $0092C9C9;
  if (DataModule1.ZConnection1.Catalog='sparepart52') then
      PanelOperasional.Color := $00808040;
