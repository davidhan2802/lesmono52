...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select g.IDgolongan,g.kode,g.nama,sum(i.qty) qty, sum(i.qty*i.hargabeli) value from product p ');
    SQL.Add('left join inventory i on p.kode=i.kodebrg ');
    ...
    SQL.Add('group by g.IDgolongan,g.kode,g.nama ');
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select p.IDgolongan,p.kode,p.barcode,p.nama,sum(i.qty) qty, sum(i.qty*i.hargabeli) value from product p ');
    SQL.Add('left join inventory i on p.kode=i.kodebrg ');
    SQL.Add('where '+strgdg+'(i.tgltrans between ''' + FormatDateTime('yyyy-MM-dd',SellTxtSearchFirst.Date) + ''' ');
    SQL.Add('and ''' + FormatDateTime('yyyy-MM-dd',SellTxtSearchLast.Date) + ''') and (p.IDgolongan=:IDgolongan) ');
    SQL.Add('group by p.IDgolongan,p.kode,p.nama ');
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select sum(i.qty*i.hargabeli) value from inventory i ');
    SQL.Add('where '+strgdg+'(i.tgltrans between ''' + FormatDateTime('yyyy-MM-dd',SellTxtSearchFirst.Date) + ''' ');
    SQL.Add('and ''' + FormatDateTime('yyyy-MM-dd',SellTxtSearchLast.Date) + ''') ');
    Open;
...
begin
 ...
 '(select coalesce(sum(y2.qty*y2.hargabeli),0) from inventory y2 where (y2.kodegudang=i.kodegudang) and (y2.kodebrg=i.kodebrg) and (y2.tgltrans<'+Quotedstr(getmysqldatestr(SellTxtSearchFirst.Date))+')) amountawal,' +
 ...
 1 :
 DataModule1.ZQrystokmutasirep.SQL.Text :=
 'select p.kode,p.nama,(select coalesce(sum(y1.qty),0) from inventory y1 where (y1.kodegudang=i.kodegudang) and (y1.kodebrg=i.kodebrg) and (y1.tgltrans<'+Quotedstr(getmysqldatestr(SellTxtSearchFirst.Date))+')) qtyawal,' +
 '(select coalesce(sum(y2.qty*y2.hargabeli),0) from inventory y2 where (y2.kodegudang=i.kodegudang) and (y2.kodebrg=i.kodebrg) and (y2.tgltrans<'+Quotedstr(getmysqldatestr(SellTxtSearchFirst.Date))+')) amountawal,' +
 'sum(if(i.qty>0,i.qty,0)) qtymasuk,sum(if(i.qty>0,i.qty,0)*i.hargabeli) amountmasuk,' +
 '-1*sum(if((i.qty<0)and(trim(i.`typetrans`)="penjualan"),i.qty,0)) qtyjual,' +
 '-1*sum(if((i.qty<0)and(trim(i.`typetrans`)="penjualan"),i.qty,0)*i.hargabeli) amountjual,' +
 ...
 'where '+strgdg+'(i.tgltrans between ' + Quotedstr(getmysqldatestr(SellTxtSearchFirst.Date)) + ' ' +
 'and ' + Quotedstr(getmysqldatestr(SellTxtSearchLast.Date)) + ') ' +
 'group by p.kode,p.nama having (qtyawal+amountawal+qtymasuk+amountmasuk+qtyjual+amountjual+qtykeluar+amountkeluar)<>0 order by p.kode; ';
...
procedure TFrmstockgudanglist.FormShow(Sender: TObject);
begin
