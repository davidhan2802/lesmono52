...
type
  Tfrmttkonsi = class(TForm)
    ...
    edt_ttno: TRzEdit;
    RzLabel15: TRzLabel;
    dt_tgl: TRzDateTimeEdit;
    RzPanel4: TRzPanel;
    RzLabel6: TRzLabel;
    edtnum_total: TRzNumericEdit;
    ...
    RzLabel3: TRzLabel;
    edtnum_subtotal: TRzNumericEdit;
    SellPanelCredit: TRzPanel;
    RzLabel11: TRzLabel;
    DBGrid: TVolgaDBGrid;
    RzLabel5: TRzLabel;
    edtnum_ongkir: TRzNumericEdit;
    edt_ket: TRzMemo;
    RzLabel1: TRzLabel;
    edt_dibuat: TRzEdit;
    RzLabel4: TRzLabel;
    edt_diterima: TRzEdit;
    ...
    procedure edtnum_ongkirChange(Sender: TObject);
  private
    oldTotal: Double;
...
procedure Tfrmttkonsi.PrintStruck(NoFaktur: string);
var
...
procedure Tfrmttkonsi.PostingJual(Faktur: string);
var
...
function Tfrmttkonsi.getNoFakturJual : string;
...
function Tfrmttkonsi.getNoFakturJualPPN : string;
var Year,Month,Day: Word;
begin
    DataModule1.ZQrySearch.Close;
...
procedure Tfrmttkonsi.CalculateGridSell;
begin
...
procedure Tfrmttkonsi.Calculatebuy;
begin
...
procedure Tfrmttkonsi.ClearForm;
begin
...
procedure Tfrmttkonsi.InsertDataMaster;
...
procedure Tfrmttkonsi.UpdateData;
var idTrans,vwkt,valamat,vkota,vpono: string;
begin
  Subtotal := getDatanum('sum(subtotal)','formbuy where ipv='+ Quotedstr(ipcomp));
  SellTxtSubTotal.Value := SubTotal;
...
procedure Tfrmttkonsi.FormShowFirst;
begin
...
procedure Tfrmttkonsi.printbarcode;
var
  FrxMemo: TfrxMemoView;
  i : integer;
begin
...
procedure Tfrmttkonsi.SellBtnAddClick(Sender: TObject);
begin
...
procedure Tfrmttkonsi.SellBtnDelClick(Sender: TObject);
begin
 if SellLblCaption.Caption = 'Input Pembelian' then ClearForm else close;
end;

procedure Tfrmttkonsi.DBGridKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_F1 then
  begin
...
procedure Tfrmttkonsi.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 DataModule1.ZQryformbuy.Close;
...
procedure Tfrmttkonsi.FormShow(Sender: TObject);
begin
...
procedure Tfrmttkonsi.cb_nama_supplierChange(Sender: TObject);
begin
 cb_kode_supplier.ItemIndex := cb_nama_supplier.ItemIndex;
...
procedure Tfrmttkonsi.inputkodebarcode(nilai:string);
var bufstr : string;
...
procedure Tfrmttkonsi.edtnum_ongkirChange(Sender: TObject);
begin
 calculatebuy;
end;
