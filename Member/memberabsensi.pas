...
// Datamodule1.ZConnection1.ExecuteDirect('delete from memberabsensi where date(tgl)=CURRENT_DATE and idmember=' + vidmember);
 ...
 S := 'insert into memberabsensi (tgl,cardno,IDselltourgroup,nogroup,namatour,jenisbus,IDuser,username,lokasi,presentfee) values (';
 for clbcount:= 0 to clb_nogroup.Items.Count-1 do
  if clb_nogroup.Checked[clbcount] then
  begin
   Datamodule1.ZQryUtil.close;
   Datamodule1.ZQryUtil.SQL.Text := 'select nogroup,namatour,jenisbus,presentfee from selltourgroup where IDselltourgroup=' + inttostr(longint(clb_nogroup.Items.Objects[clbcount]));
   Datamodule1.ZQryUtil.Open;
   ...
   vpresentfee       := Datamodule1.ZQryUtil.Fields[3].AsFloat;
   Datamodule1.ZQryUtil.Close;

   vnogroup := vnogrp;
   if rightstr(S,1)=')' then S := S + ',(';
   tglskrg := getnow;
   S := S + Quotedstr(formatdatetime('yyyy-mm-dd hh:nn:ss',tglskrg)) +','+ Quotedstr(trim(edt_cardno2.Text)) + ',' + inttostr(longint(clb_nogroup.Items.Objects[clbcount])) + ',' + Quotedstr(vnogrp) + ',' + Quotedstr(vnmtour) +',' + Quotedstr(vjnsbus) + ',' + inttostr(IDuserlogin) + ',' + Quotedstr(username) + ',' + Quotedstr(lokasigudang) + ',' + Quotedstr(Floattostr(vpresentfee)) + ')';
  end;
