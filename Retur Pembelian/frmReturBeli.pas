...
begin
FillComboBox('faktur','buymaster where isposted=1 ',RtrbeliTxtFakturbeli,false,'faktur');

//  FillComboBox('faktur','buymaster where tanggal>DATE_SUB(CURRENT_DATE, INTERVAL 181 DAY) and isposted=1 and faktur like "B' + standPos + '-%"' ,RtrbeliTxtFakturbeli,false,'faktur');
//  FillComboBox('noinvoice','buymaster where tanggal>DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY) and isposted=1 ', RtrbeliTxtFakturbeli, false, 'noinvoice');
...
//  ClearTabel('formretur where ipv='+ Quotedstr(ipcomp) );
  DataModule1.ZConnection1.ExecuteDirect('truncate formretur');
...
begin
  ...
  begin
    ...
    SQL.Add('totalretur,TotalTrans,subtotal,ppn,fakturbeli,noinvoice,pembayaran,kodegudang,isposted) values');
    ...
    SQL.Add(QuotedStr(vinvoicebeli) + ',');
    ...
    SQL.Add('(faktur,idReturBeli,kode,nama,kategori,merk,seri,keterangan,qtybeli,qtyretur,hargabeli,hargajual,diskon,diskon_rp,diskonrp,satuan,kodegudang,fakturbeli)');
    SQL.Add('select '+Quotedstr(RtrbeliTxtFaktur.Text)+','+Quotedstr(idtrans)+',kode,nama,kategori,merk,seri,keterangan,quantity,quantityretur,hargabeli,harga,diskon,diskon_rp,diskonrp,satuan,kodegudang,'+QuotedStr(RtrbeliTxtFakturbeli.Text)+' ');
...
//    SQL.Add('pembayaran = "' + RtrTxtPembayaran.Text + '",');
    ...
    SQL.Clear;
    SQL.Add('insert into returbelidetail');
    SQL.Add('(faktur,kode,nama,kategori,merk,seri,keterangan,qtybeli,qtyretur,hargabeli,hargajual,diskon,diskonrp,satuan,fakturbeli,kodegudang,hargabeli)');
    SQL.Add('select faktur,kode,nama,kategori,merk,seri,keterangan,quantity,quantityretur,hargabeli,harga,diskon,diskonrp,satuan,fakturbeli,kodegudang,hargabeli ');
...
begin
    ...
    DataModule1.ZQrySearch.SQL.Text := 'select faktur from returbelimaster where (isposted=-1) and (faktur like "RB' + RtrbeliTxtFakturbeli.Text + '%") order by idreturbeli limit 1';
...
//    DecodeDate(TglSkrg,Year,Month,Day);
    ...
    begin
      ...
      SQL.Add('where faktur like "RB' + RtrbeliTxtFakturbeli.Text + '%"');
      Open;
      if (isEmpty) or (Fields[0].IsNull) or (Fields[0].AsInteger = 0) then
        result := 'RB' + RtrbeliTxtFakturbeli.Text + '-1'
      else
        result := 'RB' + RtrbeliTxtFakturbeli.Text + '-' + inttostr(Fields[0].AsInteger+1);
      Close;
    end;
end;
...
//    SubTotal := SubTotal - ReturbeliDBGrid.Fields[11].AsFloat;
   ...
   Subtotal := getDatanum('sum(subtotal)','formretur where ipv='+ Quotedstr(ipcomp) + ' and nourut<>'+ ReturbeliDBGrid.Fields[12].AsString );
   SubTotal := SubTotal + ReturbeliDBGrid.Fields[11].AsFloat;
  end
  else SubTotal := 0;
...
{      Close;
      ...
      SQL.Add('select kode,-1*quantityretur,satuan,hargabeli,harga,'+Quotedstr(RtrbeliTxtFaktur.Text)+','+Quotedstr(vkodegdg)+','+Quotedstr(sellTxtalamat.Lines.Strings[0])+',"retur pembelian","'+idTrans+'",' + QuotedStr(UserName) +',"'+ vwkt+'","'+FormatDateTime('yyyy-MM-dd',TglSkrg)+'" from formretur where ipv='+ Quotedstr(ipcomp) + ' and quantityretur>0');
      ExecSQL;
...
begin
  ...
  begin
    ...
    begin
      ...
      SQL.Add('kategori,merk,seri,faktur,diskon,diskonrp,fakturbeli,kodegudang,hargabeli from returbelidetail');
...
begin
 ...
 begin
  ...
  begin
    ErrorDialog('Isi dahulu Retur Dari!');
    Exit;
  end;
...
begin
  ...
  begin
    ...
    SQL.Add('where faktur = ' + Quotedstr(RtrbeliTxtFakturbeli.Text) );
    Open;
    ...
    SQL.Clear;
    SQL.Add('insert into formretur ');
    SQL.Add('(ipv,kode,nama,quantity,satuan,hargabeli,harga,kodegudang,kategori,merk,seri,faktur,diskon)');
    SQL.Add('select '+Quotedstr(ipcomp)+',kode,nama,getQtysisaReturbeli(faktur,kode),satuan,hargabeli,harganondiskon,'+Quotedstr(vkodegdg)+',kategori,merk,seri,'+Quotedstr(RtrbeliTxtFaktur.Text)+',diskon from buydetail');
    SQL.Add('where faktur = ' + Quotedstr(RtrbeliTxtFakturbeli.Text) );
    ExecSQL;
