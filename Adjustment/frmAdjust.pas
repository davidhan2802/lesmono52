...
begin
  ...
  begin
    ...
    SQL.Add('(faktur,kode,nama,kategori,merk,seri,keterangan,qtybefore,qtyafter,hargabeli,hargajual,diskon,diskonrp,satuan,subtotal)');
    SQL.Add('select faktur,kode,nama,kategori,merk,seri,keterangan,quantity,quantityadjust,harga,hargajual,diskon,diskonrp,satuan,subtotal ');
...
begin
  ...
  begin
    ...
    SQL.Add('totaladjust = "' + Number2MySQL(SubTotal) + '"');
    ...
    SQL.Clear;
    SQL.Add('insert into adjustdetail');
    SQL.Add('(faktur,kode,nama,kategori,merk,seri,keterangan,qtybefore,qtyafter,hargabeli,hargajual,diskon,diskonrp,satuan)');
    SQL.Add('select faktur,kode,nama,kategori,merk,seri,keterangan,quantity,quantityadjust,harga,hargajual,diskon,diskonrp,satuan');
    SQL.Add('from formadjust where ipv='+ Quotedstr(ipcomp));
    ExecSQL;
  end;
end;
...
begin
  ...
  begin
{    AdjustTxtFaktur.Text := DataModule1.ZQryAdjustlistfaktur.Value;
    AdjustTxtTgl.Date := DataModule1.ZQryAdjustlisttanggal.Value;
    TotalTrans := DataModule1.ZQryAdjustlisttotaladjust.Value;

    oldTotal := DataModule1.ZQryAdjustlisttotaladjust.Value;
    ...
    begin
      ...
      SQL.Add('kategori,merk,seri,faktur,harganodiskon,diskon,diskonrp)');
      SQL.Add('select '+Quotedstr(ipcomp)+',kode,nama,qtybefore,qtyafter,keterangan,hargabeli,hargajual,(qtyafter - qtybefore) * hargabeli,');
      SQL.Add('kategori,merk,seri,faktur,hargabeli,diskon,diskonrp from adjustdetail');
      SQL.Add('where faktur = ''' + DataModule1.ZQryAdjustlistfaktur.Text + '''');
      ShowMessage(SQL.Text);
      ExecSQL;
    end;
    RefreshTabel(DataModule1.ZQryFormAdjust);
    SubTotal := DataModule1.ZQryAdjustlisttotaladjust.Value; }
  end;
end;
